...
__MODULE__ = "ꜱᴛɪᴄᴋᴇʀ"
__HELP__ = """
**Bantuan Untuk Sticker**

**Perintah:**
- **{0}q**  
  **Penjelasan:** Merubah text menjadi sticker.

- **{0}kang**  
  **Penjelasan:** Menambahkan sticker ke dalam pack.

- **{0}tiny**  
  **Penjelasan:** Merubah sticker menjadi kecil.

- **{0}mmf**  
  **Penjelasan:** Sticker atau foto akan diubah menjadi sticker text.
"""
...
async def _(client, message):
    if not message.reply_to_message:
        return await message.reply("Balas foto atau stiker.")
    ...
    if len(text) < 1:
        return await Tm.edit(f"Contoh : .mmf - text")
...
async def _(client, message):
    ...
    info = await message.reply(f"{prs} Ssdang diproses...", quote=True)
    ...
    if message.reply_to_message:
        ...
        async for quotly in client.get_chat_history("@QuotLyBot", limit=1):
            if not quotly.sticker:
                await message.reply(
                    f"{ggl} Tidak dapat memproses permintaan.", quote=True
                )
            else:
        ...
        if len(message.command) < 2:
            return await info.edit(f"{ggl} Balas foto atau media.")
        else:
...
async def _(client, message):
    ...
    if not (reply and (reply.media)):
        return await message.reply("Silakan balas pesan sticker.")
    Tm = await message.reply("Ssdang diproses...")
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                await Tm.edit(f"{ggl} Stiker tidak memiliki nama.")
                return
        ...
        await Tm.edit(f"{ggl} Silakan balas media foto/gif/ꜱticker.")
        return
    if media_:
        ...
        if exist is not False:
            ...
            while limit in await get_response(message, client):
                ...
                if await get_response(message, client) == "Invalid pack selected.":
                    ...
                    await asyncio.sleep(2)
                    await Tm.edit(
                        f"{brhsl} Stiker berhaꜱil ditambahkan.\n<a href=https://t.me/addstickers/{packname}>Klik Disini</a> untuk menggunakan ꜱtiker."
                    )
                    return
            await client.send_document("stickers", media_)
            await asyncio.sleep(2)

...
Saya adalah asisten otomatis. Mohon bersabar, majikan saya akan segera membalas pesan Anda.  
Harap tidak mengirim pesan berulang kali, atau sistem akan memblokir Anda secara otomatis.

⚠ Peringatan: {warn} 

<blockquote>Powered by @ThreeUserbot</blockquote>
"""
...
- **{0}setpm**  
    ...
    Mengatur konfigurasi pada pm_permit.

**Contoh menggunakan warning:**  
    Command: .setpm limit 5
"""
        ...
        if user.id not in check:
            ...
            try:
                if FLOOD[user.id] > int(pm_limit):
                    del FLOOD[user.id]
                    await message.reply(
                        "Anda telah diberi peringatan untuk tidak melakukan spam. Karena melanggar aturan, Anda kini telah diblokir secara otomatis."
                    )
                ...
                try:
                    ...
                    if pm_pic:
                        try:
                            ...
                            await set_vars(client.me.id, "PM_PIC", "https://i.ibb.co/KpKbbg9C/86e22185cc94093c709158b364bb83ee.jpg")
                    else:
...
async def _(client, message):
    ...
    vars = await get_pm_id(client.me.id)
    if user.id not in vars:
        await message.reply(f"🙏🏻 Maaf ⁣{rpk} Anda akan diblokir.")
        return await client.block_user(user.id)
    else:
        await remove_pm_id(client.me.id, user.id)
        return await message.reply(
            f"🙏🏻 Maaf Anda telah ditolak untuk menghubungi akun ini."
        )

...
**Perintah:** 
- `<code>{0}absen</code>` 
  - Untuk membuat daftar absen kamu.

**Perintah:** 
- `<code>{0}delabsen</code>` 
  - Untuk menghapus daftar absen kamu.

**Penjelasan:**
- Gunakan perintah **{0}absen** untuk mencatat kehadiran kamu, dan perintah **{0}delabsen** jika kamu ingin menghapus daftar absen yang telah dibuat.
"""
...
async def absen_command(c, m):
    ...
    try:
        ...
        if x.results:
            ...
            await m.reply(f"<b>{ggl} Tidak ada hasil dari inline bot.</b>")
    except asyncio.TimeoutError:
        await m.reply(f"<b>{ggl} Waktu habis dalam mendapatkan hasil dari inline bot.</b>")
    except Exception as e:
        await m.reply(f"<b>{ggl} Terjadi kesalahan: {e}</b>")
...
async def clear_absen_command(c, m):
    ...
    sks = await EMO.BERHASIL(c)
    prs = await EMO.PROSES(c)
    await m.reply(f"<b>{sks} ✅ Semua absen berhasil dihapus.</b>")
...
async def absen_query(c, iq):
    ...
    text = f"<b>Absen Tanggal {timestamp}</b>\n\n**Daftar Absen:** {hadir_text}\n\n</b>"

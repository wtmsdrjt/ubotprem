...
**Perintah:**
- **{0}gikes**
    - Digunakan untuk memulai proses siaran.

**Tipe:**
- **all**: untuk semua pengguna.
- **users**: untuk pengguna tertentu.
- **group**: untuk grup tertentu.
...
- **{0}send**
    - Mengirim pesan ke pengguna/grup/channel.

- **{0}autobc**
    - Mengirim pesan siaran secara otomatis.

**Query:**
- **on/off**: Mengaktifkan atau menonaktifkan fitur.
- **text**: Teks yang ingin disiarkan.
- **delay**: Menentukan jeda waktu antara pengiriman pesan.
- **remove**: Menghapus pesan yang telah dikirim.
- **limit**: Menentukan batas jumlah pesan yang akan dikirim.

**Penjelasan:**
- Gunakan perintah di atas sesuai kebutuhan Anda untuk mengelola siaran dengan lebih efektif!
"""
...
async def limit_cmd(client, message):
    ...
    msg = await message.reply(f"Sedang diproses...")
    ...
    if status and hasattr(status, "text"):
        ...
        if pjg <= 100:
            if client.me.is_premium:
                text = f"""
**Status Akun:**
- **{pong}** Status akun: **Premium**
- **{tion}** Limit check: **Akun Anda tidak dibatasi**
- **{yubot}** Ubot: **{bot.me.mention}**
"""
            else:
                text = f"""
**Status Akun:**
- Status akun: **Tidak premium**
...
async def gcast_handler(client, message):
    ...
    if command not in ["group", "users", "all"] or not text:
        ...
        return await gcs.edit(f"{message.text.split()[0]} [ᴛʏᴘᴇ] [ᴛᴇxᴛ/ʀᴇᴘʟʏ]")
    ...
    for chat_id in chats:
        if client.me.id not in gcast_progress:
            await gcs.edit(f"**Proses gcast berhasil dibatalkan!**\n- **{sks}**")
            return
    ...
    await gcs.delete()
    _gcs = f"""
**{bcs} Broadcast terkirim**

**Status:**
- **{sks} Berhasil:** {done} chat
- **{ggl} Gagal:** {failed} chat
- **Tipe:** **{command}**
"""
...
async def stopg_handler(client, message):
    ...
    if client.me.id in gcast_progress:
        gcast_progress.remove(client.me.id)
        return await message.reply(f"**Gcast berhasil dibatalkan!**\n- **{sks}**")
    else:
        return await message.reply(f"**{ggl} Tidak ada proses gcast yang sedang berlangsung.**")

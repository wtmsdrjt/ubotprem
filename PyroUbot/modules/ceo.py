...
async def promotte(client: Client, message: Message):
    ...
    anu = await eor(message, f"{prs} Sedang diproses...")
    if not user_id:
...
async def demote(client: Client, message: Message):
    ...
    sempak = await eor(message, f"{prs} Sedang diproses...")
    if not user_id:
...
async def get_link(client, message):
    ...
    try:
        ...
        await message.reply_text(f"{sks} Link : {link}", disable_web_page_preview=True)

...
__MODULE__ = "ʙʀᴀᴛᴠɪᴅᴇᴏ"
__HELP__ =  """
**Bantuan Untuk Brat Video**

**Perintah:**
- **{0}bratvideo [text]**  
  **Penjelasan:** Untuk membuat gambar text video seperti tren TikTok.
"""

async def BratVideo(text):
    if not text:
        return "Contoh : .bratvideo ko kamu cantik banget"
    if len(text) > 250:
        return " "
    ...
    try:
        for i in range(len(words)):
            ...
            if res.status_code != 200:
                raise Exception("Gagal mengambil frame dari server.")
...
async def brat_handler(client, message):
    ...
    if not text:
        await message.reply_text("Contoh : .bratvideo ko kamu cantik banget")
        return

    processing_msg = await message.reply_text("Sedang diproses...")
    ...
    if isinstance(video_path, str) and video_path.startswith("Terjadi Kesalahan."):
        ...
        await processing_msg.delete()
        await message.reply_video(video=video_path, caption="Powered by @ThreeUserbot")

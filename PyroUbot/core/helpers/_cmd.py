    ...
    def ULTRA(func):
        async def function(client, message):
            ...
            if client.me.id not in ultra_users:
                return await message.reply_text("<emoji id=6113891550788324241>❌</emoji>Maaf! Akses ini hanya untuk [SuperUltra]. Terima kasih atas pengertiannya!")
            return await func(client, message)
    ...
    def START(func):
        async def function(client, message):
            ...
            if user_id != OWNER_ID:
                ...
                buttons = [
                    [InlineKeyboardButton("👤 Lihat Profil Pengguna 👤", callback_data=f"profil {message.from_user.id}"),
                    InlineKeyboardButton("🗯 Balas Pesan 🗯", callback_data=f"jawab_pesan {message.from_user.id}")],
                ]
